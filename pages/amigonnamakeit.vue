<template>
  <div class="main">
    ...
    <div class="body">
      <Banner />
      <p>
        Enter your wallet address and we'll check if you're currently going to
        make it before we run out of ethereal's origins
      </p>
    </div>
    <div class="form">
      <div class="input">
      ...
      <div class="share">
        <a
          v-if="stats.gonnaMakeIt"
          class="btn"
          target="_blank"
          :href="`https://twitter.com/intent/tweet?text=${shareLink}`"
          ><font-awesome-icon class="icon" :icon="['fab', 'twitter']" />Tweet</a
        >
      </div>
    </div>
    <div class="stats">
...
export default {
  ...
  methods: {
    async checkAddress() {
      ...
      const res = await axios.get(
        `https://staking-api.guanaco.dev/stats/${this.address}`
      );
      ...
      this.loading = false;
      const totalboo = (
        res.data.data.stats.claimedBoo + res.data.data.stats.unclaimedBoo
      ).toFixed(2);
      var daysTilMint;
      ...
      if (totalboo >= 1200) {
        ...
        daysTilMint = Math.ceil(
          (1200 - totalboo) / (10 * res.data.data.stats.stakedGhosts)
        );
        mintsBeforeYou = res2.data.data.totals.reduce(
          (sum, ele) =>
            ele.daysTilMint < daysTilMint
              ? sum + ele.holders * Math.ceil(daysTilMint / ele.daysTilMint)
              : sum,
          0
        );
  ...
  computed: {
    shareLink() {
      return `🎉🎉I'm going to get an @Etherealswtf Origin🎉🎉%0a%0a👻- ${
        this.stats.totalGhostsStaked
      } Ghosts Staked%0a💰- ${
        this.stats.claimed + this.stats.claimable
      } $boo bucks earned%0a📅- ${
        this.stats.daysUntilMint
      } days until I can mint%0a%0aAre you gonna make it? Find out here https://guanaco.dev/amigonnamakeit`;
    },
  },
};
</script>
  ...
  .form {
    ...
    .submit {
      ...
      button {
        ...
        border: 4px solid #ad2d6b;
        border-radius: 5px;
        cursor: pointer;
        &:hover {
          background-color: #ad2d6b;
        }
      }
    }
  }
